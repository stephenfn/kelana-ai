...
export default function TripsPage() {
  ...
  // Load user info and trips on mount
  useEffect(() => {
  ...
  async function loadTrips() {
    ...
    try {
      ...
      startTransition(() => setTrips(loadedTrips));
    } catch (err) {
      setError(
  ...
  return (
    <main className="min-h-screen bg-[#f7f9f8] text-[#142c3b]">
      <div className="mx-auto max-w-5xl px-5 sm:px-8">
        <header className="flex items-center justify-between border-b border-[#d8e4e3] py-5">
          <Link href="/" className="flex items-center gap-3">
            <span className="brand-mark">K</span>
            <span className="font-bold text-[#123b50]">KelanaAI</span>
          </Link>
          <div className="flex items-center gap-4">
            {userName && (
              <span className="text-sm text-[#587077]">
                Welcome back, <strong>{userName}</strong>
              </span>
            )}
            <Link href="/" className="text-sm font-bold text-[#168b82]">
              + New trip
            </Link>
            <button
        ...
        <section className="py-12 sm:py-16">
          ...
          <div className="mt-3 flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            <div>
              <h1 className="text-5xl font-black tracking-[-0.05em] text-[#102c3e]">
                Trip history
              </h1>
              <p className="mt-3 text-[#587077]">
                Every route you have shaped with KelanaAI.
              </p>
            </div>
            <span className="rounded-full bg-[#e2f3ee] px-4 py-2 text-sm font-bold text-[#17786f]">
              {trips.length} saved{" "}
              {trips.length === 1 ? "itinerary" : "itineraries"}
            </span>
          </div>
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row">
            ...
            <select
              ...
              className="search-input sm:w-48"
          ...
          {loading && (
            <div className="empty-state mt-6">
              ...
              <strong>Loading your journeys...</strong>
            </div>
          )}

          {error && (
            <div className="result-message mt-6 border border-[#f0c7b7] bg-[#fff4ee] text-[#a14b35]">
              <strong>We hit a small detour.</strong>
              <span>{error}</span>
              <button
          ...
          {!loading && !error && visibleTrips.length === 0 && (
            <div className="empty-state mt-6">
              ...
              {!query && (
                <Link href="/" className="font-bold text-[#168b82] underline">
